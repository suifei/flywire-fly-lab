...
const dt = 0.005;
// 找地方：平地 / 陡坡
let flat = null, steep = null; for (let x = -300; x < 300 && !(flat && steep); x += 3) for (let y = -300; y < 300; y += 3) { const s = Math.hypot(...W.grad(x, y)); let clear = true; W.near(x, y, 18, it => { if (it.solid || it.kind === "hollow") clear = false; }); if (!clear) continue; if (!flat && s < 0.03) flat = [x, y]; if (!steep && s > 0.3) steep = [x, y]; }
