...
FINDINGS = [
    ...
    dict(id="nature_world", what="把篮球场换成一个有重力、有天气、没有边界的 3D 大自然，物理对不对、果蝇在里面过得怎么样",
         ...
         result="九条事先写好的物理判据全过：30 mm 自由落体 0.08 s（解析值 0.0782 s）；弹起高度 3.09 → 0.19 mm，不生能量；"
                "坡度 0.33 的陡坡上 1 秒滚落 5.92 mm、缓坡上停得住；石头不可穿透；风把气味吹向下风（下风 0.811 对上风 0.064）；闭环里走 60 秒从不穿进实心物件。"
                "在里面过 3 分钟（3 个种子均值）：走 3229 mm、被石头之类挡住 4.1 s、起飞 6.7 次、掉下来 19.3 颗浆果、碰到糖 2.7 次（平地开放世界 2.3 次）",
         ...
         verify=[("all_pass", True, 0), ("tests.N1_free_fall.t_sim", 0.08, 0.0001), ("tests.N4_rolls_downhill_only_when_steep.dropped_mm", 5.92, 0.01), ("tests.N9_closed_loop_walk.max_penetration_mm", 0, 1e-6)]),
