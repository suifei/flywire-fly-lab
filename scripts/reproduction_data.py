...
FINDINGS = [
    ...
    dict(id="eco_surface", what="生态箱：把 15,055 个神经元的真脑蒸馏成响应面（27 路输入 → 12 个读出），好让 32 只果蝇的进化跑得动",
         status="reproduced",
         result="真脑一只 1.18 倍实时、32 只 0.037 倍。用真脑跑 7,200 组输入拟合；留出集（单次试验、原始 Hz）R²：左转 0.914、右转 0.904、巨纤维 0.993、MN9 0.942（事先定的线 ≥ 0.85）。"
         ...
         verify=[("surface.pass", 1, 0), ("surface.n_samples", 7200, 0), ("surface.features.0.r2", 0.914, 0.0001), ("surface.features.2.r2", 0.9925, 0.0001)]),
    dict(id="eco_m2_learning", what="生态箱 M2：只给物理量和内生奖励（身体不适的减少），接在固定连接组旁边的可塑性层能不能自己学会活得更久",
         status="partial",
         result="3 个个体各连续 30 条命后冻结学习，在 30 个没见过的世界上测：不学习 444 s，学习 ≥ 1800 s（截尾，× 4.05）；C1（≥ 1.5 ×）3/3 通过，C2（打乱奖励不提升）3/3 通过；另换 10 个个体 10/10 学成；长期训练任务（eco/train_long.js，又 10 个新个体，学成即毕业）10/10 学成、换新世界复测 10/10，共 130 条命。"
                "学到的是「碰到水就停下来喝」（连接组里水味先天不够伸喙）和「湿度上升时少拐弯」",
         caveat="标 partial 的原因：**远距离找水 / 找食物没学会**（干旱世界不学习 419 s、学习 397 s，训练 120 条命 516 s）；可塑性层看不到大脑输出时学得一样好（× 4.05）——连接组在这里给的是身体和反射，不是学习信号。"
                "第一版 C2 只过了 2/3（打乱对照是漏的：奖励只延后 20–60 s，与持续几十秒的喝水仍相关；事后加的轭式对照不提升）。关掉「不用就忘」10 个个体只有 5/10 学成（1 个贴墙、3 个躺平）。寿命在 1,800 s 截尾，倍数是下限",
         script="eco/m2_learning.js", result_file="results/eco/summary.json", log="§50.4",
         verify=[("m2.C1", 1, 0), ("m2.C2", 1, 0), ("m2.individuals.0.ratio_on", 4.05, 0.001), ("m2.robust_learned", 10, 0), ("long.n_learned", 10, 0), ("long.n_retest_ok", 10, 0), ("m2.drought_on", 397, 0)]),
    dict(id="eco_m3_evolution", what="生态箱 M3：32 只果蝇、10 个可遗传参数、没有适应度函数——不同的世界规则会不会把「飞行倾向」推向不同方向",
         status="reproduced",
         result="三个世界各 5 个种子 × 10,800 s：甲虫横行 降（Δ 均值 -1.15，寿命 401 → 883 s，先天喝水 +2.32）；风平浪静 平（Δ 均值 -0.29，寿命 481 → 1164 s，先天喝水 +2.48）；地广物稀 平（Δ 均值 +0.06，寿命 401 → 431 s，先天喝水 +0.67）。事先写定的判据（至少两个世界标签不同）通过",
         ...
         verify=[("m3.pass", 1, 0), ("m3.worlds.0.delta_mean", -1.151, 0.001), ("m3.worlds.1.delta_mean", -0.293, 0.001), ("m3.worlds.2.delta_mean", 0.055, 0.001)]),
    dict(id="eco_live_check", what="生态箱：在响应面上学成的个体，放回真的 15,055 神经元脉冲网络里还管不管用",
         status="partial",
         result="3 个世界种子、上限 900 s：真脑 900 s，响应面 900 s，白纸 419 s。L1（真脑 ≥ 1.5 × 白纸）通过（× 2.15）；L2（喝水时间占比在响应面的 0.5–2 倍内）未通过（0.284 对 0.073）",
         ...
         verify=[("live.live_median", 900, 0), ("live.naive_median", 419, 0)]),
    dict(id="eco_transfer", what="生态箱学成的个体搬进 3D 大自然（真脑 + 有重力天气甲虫的世界）还管不管用；训练条件先对齐到大自然",
         status="reproduced",
         result="5 个世界种子 × 900 s，同种子对比白纸，权重冻结；有信息的种子 5 个。X1（喝得更久 5/5，每次碰到水喝 6.583333333333333 对 0.7777777777777778 次）通过；X2（更不渴 5/5）通过。"
                "响应面第三版加了闭环工况（11,339 组，含大自然里记下的 9,000 组）：只尝到水的 MN9 误差 2.9%（第二版约 40%），真脑指数 86.6",
         caveat="学成的只是「碰到水就喝」与趋湿；大自然里能喝的水（洼地）少，5 个种子里有的臂整条命没碰到水。响应面 T2 未通过（左转 DNa 0.829，闭环工况上它的噪声天花板实测 0.827，线定得高于真脑的可重复性，照原线登记）。旧感觉编码下的 M2 / M3 / 长期训练结果留在本机 old_senses/，不入库",
         script="eco/nature_transfer.js", result_file="results/eco/summary.json", log="§50.7b",
         verify=[("transfer.pass", 1, 0), ("transfer.n_informative", 5, 0), ("surface.manifold.index", 86.6, 0.01)]),
    dict(id="eco_m4_save", what="生态箱 M4：存档 → 清空 → 读回 → 接着跑，是否与不中断逐位相同",
         status="reproduced",
         result="四项全过：续跑状态指纹相同（0c5cef30:230564）、存档格式稳定、挑战码（167 个字符）来回无损、坏输入被拒绝。页面里再验一次（eco/page_test.js）",
