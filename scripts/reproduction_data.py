...
FINDINGS = [
    ...
    dict(id="eco_m2_learning", what="生态箱 M2：只给物理量和内生奖励（身体不适的减少），接在固定连接组旁边的可塑性层能不能自己学会活得更久",
         status="partial",
         result="3 个个体各连续 30 条命后冻结学习，在 30 个没见过的世界上测：不学习 425 s，学习 ≥ 1800 s（截尾，× 4.231）；C1（≥ 1.5 ×）3/3 通过，C2（打乱奖励不提升）3/3 通过；另换 10 个个体 9/10 学成；长期训练任务（eco/train_long.js，又 10 个新个体，学成即毕业）10/10 学成、换新世界复测 10/10，共 210 条命。"
                "学到的是「碰到水就停下来喝」（连接组里水味先天不够伸喙）和「湿度上升时少拐弯」",
         caveat="标 partial 的原因：**远距离找水 / 找食物没学会**（干旱世界不学习 417 s、学习 422 s，训练 120 条命 506 s）；可塑性层看不到大脑输出时学得一样好（× 4.231）——连接组在这里给的是身体和反射，不是学习信号。"
                "第一版 C2 只过了 2/3（打乱对照是漏的：奖励只延后 20–60 s，与持续几十秒的喝水仍相关；事后加的轭式对照不提升）。关掉「不用就忘」10 个个体只有 6/10 学成（1 个贴墙、2 个躺平）。寿命在 1,800 s 截尾，倍数是下限",
         script="eco/m2_learning.js", result_file="results/eco/summary.json", log="§50.4",
         verify=[("m2.C1", 1, 0), ("m2.C2", 1, 0), ("m2.individuals.0.ratio_on", 4.231, 0.001), ("m2.robust_learned", 9, 0), ("long.n_learned", 10, 0), ("long.n_retest_ok", 10, 0), ("m2.drought_on", 422, 0)]),
    dict(id="eco_m3_evolution", what="生态箱 M3：32 只果蝇、10 个可遗传参数、没有适应度函数——不同的世界规则会不会把「飞行倾向」推向不同方向",
         status="reproduced",
         result="三个世界各 5 个种子 × 10,800 s：甲虫横行 降（Δ 均值 -1.54，寿命 408 → 926 s，先天喝水 +2.63）；风平浪静 平（Δ 均值 -0.37，寿命 451 → 930 s，先天喝水 +1.63）；地广物稀 平（Δ 均值 -0.56，寿命 406 → 481 s，先天喝水 +0.39）。事先写定的判据（至少两个世界标签不同）通过",
         ...
         verify=[("m3.pass", 1, 0), ("m3.worlds.0.delta_mean", -1.542, 0.001), ("m3.worlds.1.delta_mean", -0.371, 0.001), ("m3.worlds.2.delta_mean", -0.563, 0.001)]),
    dict(id="eco_live_check", what="生态箱：在响应面上学成的个体，放回真的 15,055 神经元脉冲网络里还管不管用",
         status="reproduced",
         result="3 个世界种子、上限 900 s：真脑 900 s，响应面 900 s，白纸 422 s。L1（真脑 ≥ 1.5 × 白纸）通过（× 2.13）；L2（喝水时间占比在响应面的 0.5–2 倍内）通过（0.199 对 0.134）",
         ...
         verify=[("live.live_median", 900, 0), ("live.naive_median", 422, 0)]),
    dict(id="eco_transfer", what="生态箱学成的个体搬进 3D 大自然（真脑 + 有重力天气甲虫的世界）还管不管用；训练条件先对齐到大自然",
         status="reproduced",
         result="5 个世界种子 × 900 s，同种子对比白纸，权重冻结；有信息的种子 5 个。X1（喝得更久 5/5，每次碰到水喝 7.111111111111111 对 0.7777777777777778 次）通过；X2（更不渴 5/5）通过。"
    ...
    dict(id="eco_m4_save", what="生态箱 M4：存档 → 清空 → 读回 → 接着跑，是否与不中断逐位相同",
         status="reproduced",
         result="四项全过：续跑状态指纹相同（1f6c4944:245817）、存档格式稳定、挑战码（167 个字符）来回无损、坏输入被拒绝。页面里再验一次（eco/page_test.js）",
