...
T = LG["tests"]
CF = L.get("court_five"); court5 = ""
if CF:
    c, o = CF["court"], CF["open"]
    court5 = f"""### 48.8 把它搬进 3D 篮球场：球场里的「五感全开」（页面；`learn/court_five.js`）
...
搬进有边界的球场之后过得怎么样（v5 + 六条腿，世界自己运转，{CF['seconds']} s × {CF['seeds']} 个种子的均值；只报告，不设判据）：

| | 篮球场（320 × 172 mm，触感 + 围栏视觉） | 没有墙的开放世界 |
|---|---|---|
| 总路程 | {c['path_mm']:.0f} mm | {o['path_mm']:.0f} mm |
| 贴墙时间 | {c['wall_frac'] * 100:.0f}% | {o['wall_frac'] * 100:.0f}% |
| 起飞 / 躲开 / 被砸 | {c['jumps']:.1f} / {c['dodge']:.1f} / {c['hit']:.1f} | {o['jumps']:.1f} / {o['dodge']:.1f} / {o['hit']:.1f} |
| 碰到糖 / 吃了几秒 | {c['sugar_contacts']:.1f} / {c['feed_s']:.1f} s | {o['sugar_contacts']:.1f} / {o['feed_s']:.1f} s |
| 多巴胺：PAM / PPL1 | {c['pam_s']:.1f} s / {c['ppl1_s']:.1f} s | {o['pam_s']:.1f} s / {o['ppl1_s']:.1f} s |
| 3 分钟后的记忆：A 的奖赏 / B 的惩罚 | {c['memA_reward'] * 100:.0f}% / {c['memB_punish'] * 100:.0f}% | {o['memA_reward'] * 100:.0f}% / {o['memB_punish'] * 100:.0f}% |

§47.6 的局限原样带进来了：开着围栏视觉它不贴墙（{c['wall_frac'] * 100:.0f}%），代价是 3 分钟起飞 {c['jumps']:.0f} 次（开放世界 {o['jumps']:.0f} 次）——自己朝围栏走过去就被当成逼近，白白起飞；关掉围栏视觉只靠触感则避不开墙。球场版那只本来就是这样，页面上「围栏视觉」那个开关对它同样有效。没有为了让它在球场里好看去加「碰墙就掉头」之类的规则。
