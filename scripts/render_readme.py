...
- `README.zh-CN.md`（中文）：本脚本负责顶部的语言切换、导言与主要结论第 1–4 条（<!-- intro -->）、
  「它做不到什么」表（<!-- limits -->）、「作者自查出的错误」（<!-- audit -->）；其余几节仍由 scratch/patch_readme_*.py 渲染。
- `README.md`（英文，GitHub 默认显示）：**整份**由本脚本渲染；中文版是 `README.zh-CN.md`。数字与中文版取自同一批结果文件，不经人手。
...
# ───────────────────────── 中文：导言 / 做不到什么 / 错误账本 ─────────────────────────
ZH_SWITCH = "[English](README.md) · **中文**\n"
...
由脚本自动产出的数字没有查出错——**错误几乎全部集中在手写的汇总与跨章节引用上**。所以网页上的表格与两份 README 一律从结果文件读取，不写死（`python3 scripts/render_readme.py --check` 在 CI 里核对）。
<!-- audit:end -->
"""
...
def patch_zh(s):
    ...
    if "| `i18n/` |" not in s: s = s.replace("| `docs/log/report.md` |", "| `i18n/` | 页面中英双语：运行时翻译层、从构建产物抽中文目录、词典与译名表、英文模式残留扫描 |\n| `docs/log/report.md` |", 1)
...
> **Start with [`REPRODUCTION.en.md`](REPRODUCTION.en.md)** ([中文](REPRODUCTION.md)): every claim of each paper we checked, what we got, and what we could not do.
...
> The web pages follow your browser language (English or Chinese; there is a switch in the corner). The code comments and the lab notebook are in Chinese. This README is generated by `scripts/render_readme.py` from the same result files as the [Chinese README](README.zh-CN.md), so the numbers match. Issues in English are welcome.
...
No error was found in numbers produced by scripts — **the errors were almost all in hand-written summaries and cross-references**. That is why the web pages and both READMEs read their numbers from result files (`python3 scripts/render_readme.py --check` runs in CI).
...
| `i18n/` | Bilingual pages: the runtime translator, the Chinese-string catalogue extracted from the built pages, the dictionary and glossary, and the English-mode residue scan |
...
def main():
    ...
    zp = ROOT / "README.zh-CN.md"; z0 = zp.read_text(); z1 = patch_zh(z0)
    ep = ROOT / "README.md"; e0 = ep.read_text() if ep.exists() else ""
    if check:
        if z1 != z0: bad.append("README.zh-CN.md 与结果文件脱节（重跑 python3 scripts/render_readme.py）")
        if EN != e0: bad.append("README.md（英文）与结果文件脱节（重跑 python3 scripts/render_readme.py）")
        for m in re.findall(r"\{[a-zA-Z_][^{}\n]{0,40}\}", EN): bad.append("README.en.md 里有没替换的占位：" + m)
        for b in bad: print("✗", b)
        if not bad: print("✓ README.md（英文）/ README.zh-CN.md（中文）与结果文件一致")
        sys.exit(1 if bad else 0)
    zp.write_text(z1); ep.write_text(EN); print("→ README.md（英文，整份）\n→ README.zh-CN.md（中文：导言 / 结论 1–4 / 做不到什么 / 错误账本）")
