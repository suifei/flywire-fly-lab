...
<!-- intro:begin （本节由 scripts/render_readme.py 从结果文件渲染）-->
**把果蝇全脑连接组跑在一台 16 GB 笔记本上，接上身体，做成游戏，再对它做 24,250 次虚拟手术。**
完整记录能做到什么、**做不到什么**，以及作者自查出的每一处错误。

[**▶ 在线试玩（浏览器里 15,055 个真实神经元实时放电，生活在一个 3D 大自然里）**](https://suifei.github.io/flywire-fly-lab/game.html) · [**▶ 生态箱**](https://suifei.github.io/flywire-fly-lab/ecobox.html) · [**完整报告 docs/log/report.md**](docs/log/report.md) · [**数据许可 ⚠**](DATA_LICENSE.md)
<!-- intro:end -->
...
<!-- what:begin （本节由 scripts/render_readme.py 从结果文件渲染）-->
## 这是什么
...
| 跑一秒大脑 | 2.5 秒（Brian2，糖刺激 1 s） |
| 虚拟敲除筛选 | **24,250 段**全脑仿真，14.68 小时机时，3 条感觉通路 |
| 浏览器里的子回路 | 球场版 5,563 个神经元 / 432,437 条连接；生活与大自然 15,055 个 / 814,649 条（含蘑菇体） |
...
**3. 同一个动作，背后是两套线路。**
糖和水都通过同一个运动神经元 MN9 引发伸喙，但活跃神经元只重叠三分之一，枢纽各自私有：`CB0883` 在水通路里根本不放电，`CB0051` 在糖通路里敲了毫无影响。抑制性神经元 `Phantom` 敲掉后水通路输出**翻 3.0 倍**（全脑，零假设中位数归一化）。
...
<!-- eco:begin （本节由 scratch/patch_readme_eco.py 渲染）-->
...
| 连接组 | 15,055 个神经元，逃逸、伸喙、转向、后退这些反射是它自带的 | **没有人** |

- **单只学习**：同一只连续 30 条命后冻结学习，在没见过的世界里测——不学习 425 s，学过的 ≥ 1800 s（× 4.231，截尾）；打乱奖励时序的对照不提升；换 10 个个体 9/10 学成；**长期训练任务再换 10 个新个体：10/10 学成，换新世界复测 10/10**（`node eco/train_long.js`，共 210 条命）。
  它学到的是「碰到水就停下来喝」（连接组里水味只把伸喙神经元推到十几 Hz，先天不够）和「湿度上升时少拐弯」。**没学会**远距离找水：干旱世界里 422 s 对 417 s。
- **32 只进化**：没有适应度函数，吃饱喝足的才攒得出卵。甲虫横行：寿命 408 → 926 s，飞行倾向降；风平浪静：寿命 451 → 930 s，飞行倾向平；地广物稀：寿命 406 → 481 s，飞行倾向平。事先写定的判据（不同世界把飞行倾向推向不同方向）通过。
  飞行是**可决策**的技能：起不起飞它说了算，怎么飞是固定弹道——飞行控制没接进连接组，不假装它能学会飞。
- **发现卡**：躺平、贴着玻璃走、原地打转、守着食物不走……奖励作弊和失败都会被抓出来、配上证据。你来选：接受它现在的活法，或者改世界，逼它换一种。
- **存档就是一份训练成果**：世界规则 + 世代时间线 + 每只果蝇的权重 + 发现卡 + 回放。读回来接着跑与不中断逐位相同；挑战码（167 个字符）发给别人就能在同一个世界里开局。
- 页面里跑的是真脑的**响应面**（真脑一只才 1.18 倍实时）：7,200 组真脑采样拟合，留出 R² 转向 0.91 / 0.90、巨纤维 0.99。学成的个体放回真脑复核：900 s 对白纸 422 s（管用）；真脑里喝水的时间占比是响应面的 1.49 倍，事先定的「行为量级一致」（0.5–2 倍）通过。

- **搬进 3D 大自然**：训练条件先对齐到大自然（同一套感觉编码；响应面加上闭环工况，只尝到水的 MN9 误差从约 40% 降到 2.9%），学成的个体冻结权重放进实验室页的大自然（真脑）：5 个有信息的种子里喝得更久 5/5、更不渴 5/5，事先写定的判据通过。实验室页「生活」的大自然里可以「载入生态箱存档」。
...
| 糖 / 苦 / 热 / 湿能靠真实接线叫起多巴胺神经元吗 | **阴性**：12 个条件里发放 ≥5 Hz 的多巴胺神经元一个都没有。所以「吃到糖 → PAM、烫 → PPL1」这根线是手接的（等价于实验里的光遗传激活） |
...
| 记住了，会改变行为吗 | **阴性**：训练后各测 12 次，6 组运动读出没有一组显著变化。有运动落点的那几类 MBON 被 APL 压死（APL 在真果蝇里不放电，模型把它当放电神经元）。页面上的「记忆 → 转向的桥」是手写的、默认关 |
...
<!-- limits:begin （本节由 scripts/render_readme.py 从结果文件渲染）-->
## 它做不到什么（同样重要）

| 想做的事 | 实际结果 |
|---|---|
| 从感光细胞（R1-6/R7/R8）驱动行为 | 下行神经元**全部 0 Hz**，信号传不过去；必须从 LC4 这类视觉投射神经元注入 |
| 吃到糖产生奖赏信号 | 糖 / 苦 / 热 / 湿 12 个条件里，发放 ≥ 5 Hz 的多巴胺神经元**一个都没有**。蘑菇体学习用的强化信号是手接的线（等价于光遗传激活）；生态箱里的奖励是身体的内生奖励，在连接组外面算 |
| 腹神经索产生走路节律 | 有节律，但凑不出三角步态；六条腿里实际只有一条在动。页面上六条腿的节律是手写的 |
| 闻到气味往哪边转 | 单侧刺激给不出左右差异（全脑，偏侧指数 -0.008 / -0.061）。原来一给气味就上万个神经元失控，已查清是触角叶局部神经元的递质符号（改正后失控 6/6 → 0/6），但左右差异仍然没有 |
| 记住了就改变行为 | 蘑菇体里记忆**形成得了**（§48），但**走不到运动输出**：训练后 6 组运动读出**没有一组**显著变化。生态箱里学会生存，靠的是连接组旁边手写的可塑性层，不是连接组本身 |
| 闭环行为（大脑 + 身体） | 做了，**但行为映射是手写的**（DNa → 转向、巨纤维 → 起跳、MN9 → 进食的阈值与增益）。物理身体闭环里奖赏记忆成立、惩罚记忆不成立（§48）；生态箱学成的个体搬进 3D 大自然：喝得更久 5/5、更不渴 5/5（§50） |
| 让果蝇只靠眼睛躲球 | 做了，**是负结果且可量化**：真实像素 → flyvis → LPLC2 这条链路在 **45 mm 外读数恒为 0**，25 mm 以内才把球从噪声里分出来；而果蝇**光是走路**，自体运动的光流就造出峰值 **126 Hz** 的假逼近（[§28.19](docs/log/report.md)） |
<!-- limits:end -->

<!-- audit:begin （本节由 scripts/render_readme.py 渲染）-->
## 作者自查出的错误

[docs/log/report.md §23](docs/log/report.md) 里有一份**错误账本**，记录所有自己发现、又自己改掉的错误，不删。三轮逐数字核对（对照结果文件）共查出 **48 处**问题（第三轮从最新结论倒查到最早，新查出 20 处）。典型的有：
...
```bash
# 1. 环境（几个 conda 环境不能合并，见 setup.sh）
...
# FlyWire 注释表、Shiu 补充材料等的获取方式见 DATA_LICENSE.md 与 docs/log/report.md

# 3. 最小实验：刺激糖味觉神经元，看伸喙运动神经元 MN9
conda activate brain-fly-cpu
python run_experiment.py --preset sugar --t_run 1
#   → 398 个活跃神经元；MN9 左 98 Hz / 右 69 Hz（官方笔记本的左右命名）；其余下行神经元全 0
...
| `eco/` | 生态箱：世界、身体与内生奖励、可塑性层、响应面、进化、存档、迁移到 3D 大自然 |
...
| `i18n/` | 页面中英双语：运行时翻译层、从构建产物抽中文目录、词典与译名表、英文模式残留扫描 |
