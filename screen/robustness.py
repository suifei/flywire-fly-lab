...
def paper_11(sheet):
    ...
    fr = [i for i, v in enumerate(hdr) if "Firing Rate" in str(v)][0]
...
def cmd_analyze():
    ...
    mn9 = [int(r) for r, t in zip(ann.root_id, ann.cell_type) if t == "MN9" and int(r) in fid2i]
    side = {int(r): s for r, s in zip(ann.root_id, ann.side)}
    mn9l = [fid2i[f] for f in mn9 if side.get(f) == "left"]
    mn9r = [fid2i[f] for f in mn9 if side.get(f) == "right"]
    cnt = {}
    for kind in PERTURB:
        f = WDIR / "chunks" / f"{kind}.npz"
        if not f.exists():
    ...
    print(f"  MN9 左 {base[:, mn9l].mean():.1f} Hz / 右 {base[:, mn9r].mean():.1f} Hz")
    ...
    for kind in PERTURB:
        ...
        rec = dict(n_active=len(resp), kept_from_baseline=keep,
                   ...
                   mn9_left=round(l, 2), mn9_right=round(r, 2), P2_ipsi_weaker=bool(p2))
        cmpstr = ""
        if kind in SHEET:
            try:
