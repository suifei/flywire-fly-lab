...
它自己排出的顺序（成五 > 活四 > 冲四、活三 > …）全部正确，第一选择与深度 8 搜索一致的比例 **45.1%**（随机 1.5%、不经过脑子 34.8%）。
把这张表交给只懂规则的搜索，想 6 步对搜 4 步的手写老师 **26–14**、想 10 步对搜 8 步的老师 **13–7**。
**但**：打乱接线（47.4%）和同维度的随机网络（49.6%）都不输它，正面交锋 17–23——
...
| 自对弈强化（「多巴胺」式三因子规则）让它越下越强 | **阴性**：强化后的表对强化前 78–122（39.0%，事先定的线是 55%）。而且奖励预测误差是在连接组**外面**算的——这个模型自己的多巴胺神经元不放电（[§46.7](docs/log/report.md)） |
