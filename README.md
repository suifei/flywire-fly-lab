...
- **单只学习**：同一只连续 30 条命后冻结学习，在没见过的世界里测——不学习 460 s，学过的 ≥ 1800 s（× 3.913，截尾）；打乱奖励时序的对照不提升；换 10 个个体 10/10 学成；**长期训练任务再换 10 个新个体：10/10 学成，换新世界复测 10/10**（`node eco/train_long.js`，共 100 条命）。
  它学到的是「碰到水就停下来喝」（连接组里水味只把伸喙神经元推到十几 Hz，先天不够）和「湿度上升时少拐弯」。**没学会**远距离找水：干旱世界里 386 s 对 383 s。
- **32 只进化**：没有适应度函数，吃饱喝足的才攒得出卵。甲虫横行：寿命 445 → 1841 s，飞行倾向平；风平浪静：寿命 604 → 2779 s，飞行倾向平；地广物稀：寿命 617 → 1339 s，飞行倾向降。事先写定的判据（不同世界把飞行倾向推向不同方向）通过。
...
- 页面里跑的是真脑的**响应面**（真脑一只才 1.18 倍实时）：7,200 组真脑采样拟合，留出 R² 转向 0.93 / 0.91、巨纤维 0.99。学成的个体放回真脑复核：900 s 对白纸 469 s（管用）；但真脑里喝水的时间占比是响应面的 4.07 倍，事先定的「行为量级一致」那条**没过**——响应面是蒸馏，不是真脑。
