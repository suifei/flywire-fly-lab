...
- 页面里跑的是真脑的**响应面**（真脑一只才 1.18 倍实时）：7,200 组真脑采样拟合，留出 R² 转向 0.91 / 0.90、巨纤维 0.99。学成的个体放回真脑复核：900 s 对白纸 422 s（管用）；但真脑里喝水的时间占比是响应面的 1.49 倍，事先定的「行为量级一致」那条通过——响应面是蒸馏，不是真脑。
