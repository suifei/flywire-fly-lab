...
- **单只学习**：同一只连续 30 条命后冻结学习，在没见过的世界里测——不学习 444 s，学过的 ≥ 1800 s（× 4.05，截尾）；打乱奖励时序的对照不提升；换 10 个个体 10/10 学成；**长期训练任务再换 10 个新个体：10/10 学成，换新世界复测 10/10**（`node eco/train_long.js`，共 130 条命）。
  它学到的是「碰到水就停下来喝」（连接组里水味只把伸喙神经元推到十几 Hz，先天不够）和「湿度上升时少拐弯」。**没学会**远距离找水：干旱世界里 397 s 对 419 s。
- **32 只进化**：没有适应度函数，吃饱喝足的才攒得出卵。甲虫横行：寿命 401 → 883 s，飞行倾向降；风平浪静：寿命 481 → 1164 s，飞行倾向平；地广物稀：寿命 401 → 431 s，飞行倾向平。事先写定的判据（不同世界把飞行倾向推向不同方向）通过。
...
- 页面里跑的是真脑的**响应面**（真脑一只才 1.18 倍实时）：7,200 组真脑采样拟合，留出 R² 转向 0.91 / 0.90、巨纤维 0.99。学成的个体放回真脑复核：900 s 对白纸 419 s（管用）；但真脑里喝水的时间占比是响应面的 3.89 倍，事先定的「行为量级一致」那条**没过**——响应面是蒸馏，不是真脑。
