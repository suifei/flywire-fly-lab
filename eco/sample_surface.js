...
const k = +(process.argv[2] || 0), K = +(process.argv[3] || 1), N = +(process.argv[4] || 900), REPS = +(process.argv[5] || 1), FROM = process.argv[6] ? JSON.parse(fs.readFileSync(path.resolve(process.argv[6]), "utf8")).X : null;   /* 第 6 个参数 = 输入向量文件（闭环工况，collect_manifold.js 写的）：第 k 片取其中 i % K === k - K0 的那些，写 surface_manifold_<k>.json */   /* REPS > 1：同一组输入换种子重复，写 surface_test_<k>.json（量噪声天花板用，不参与训练） */
...
let MINE = FROM ? FROM.filter((_, i) => i % K === k % K) : null;
if (MINE && process.env.SKIP_DONE) { const done = new Set(); for (const f of fs.readdirSync(path.join(ROOT, "results/eco")).filter(f => /^surface_manifold_\d+\.json$/.test(f))) for (const v of JSON.parse(fs.readFileSync(path.join(ROOT, "results/eco", f), "utf8")).X) done.add(v.join(",")); MINE = MINE.filter(v => !done.has(v.map(q => +q.toFixed(1)).join(","))); console.log(`片 ${k}: 跳过已答过的，剩 ${MINE.length}`); }   /* SKIP_DONE=1：只答还没答过的输入（增量），写到新的片号里 */
const NN = MINE ? MINE.length : N;
for (let i = 0; i < NN; i++) {
  ...
  if (MINE) { CH.INPUTS.forEach((c, j) => { x[c] = MINE[i][j]; }); }
  else if (fams.some(f => f[0] === "lc4") && rnd() < 0.7) { x.lplc2_L = x.lc4_L * (0.3 + 0.9 * rnd()); x.lplc2_R = x.lc4_R * (0.3 + 0.9 * rnd()); }          // 逼近时 LC4 与 LPLC2 总是一起来
  const reps = [];
  ...
  if (i % 100 === 99) console.log(`片 ${k}: ${i + 1}/${NN}  ${((Date.now() - t0) / 1000).toFixed(0)} s`);
}
fs.writeFileSync(path.join(ROOT, `results/eco/surface_${MINE ? "manifold" : REPS > 1 ? "test" : "samples"}_${k}.json`), JSON.stringify(Object.assign({ inputs: CH.INPUTS, features: CH.FEATURES, X, Y, settle_ms: 100, read_ms: 300 }, REPS > 1 ? { YREP, reps: REPS } : {})));
