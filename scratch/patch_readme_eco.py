...
S = json.loads((ROOT / "results/eco/summary.json").read_text()); m2, m3, m4, lv, sf, au = (S[k] for k in ("m2", "m3", "m4", "live", "surface", "audit")); i0 = m2["individuals"][0]; lg = S["long"]
...
- 页面里跑的是真脑的**响应面**（真脑一只才 {au['realtime_one']} 倍实时）：{sf['n_samples']:,} 组真脑采样拟合，留出 R² 转向 {sf['features'][0]['r2']:.2f} / {sf['features'][1]['r2']:.2f}、巨纤维 {sf['features'][2]['r2']:.2f}。学成的个体放回真脑复核：{lv['live_median']} s 对白纸 {lv['naive_median']} s（管用）；但真脑里喝水的时间占比是响应面的 {lv['drink_ratio']} 倍，事先定的「行为量级一致」那条{'通过' if lv['L2'] else '**没过**'}——响应面是蒸馏，不是真脑。
