...
身体由**六条腿**推着走（支撑脚的运动学解出身体的位移和转角），可以截腿、可以按住按钮直接遥控某几个神经元。判据都写在跑之前：
...
| 糖 / 苦 / 热 / 湿能靠真实接线叫起多巴胺神经元吗 | **阴性**：{rf['n_conditions']} 个条件里发放 ≥5 Hz 的多巴胺神经元最多 {rf['max_dan']} 个。所以「吃到糖 → PAM、烫 → PPL1」这根线是手接的（等价于实验里的光遗传激活） |
...
| 记住了，会改变行为吗 | **阴性**：训练后各测 {M['v5']['n_test']} 次，五个运动读出显著变化的组数最多 {M['v5']['max_hits']} / {M['v5']['n_rows']}。有运动落点的那几类 MBON 被 APL 压死（APL 在真果蝇里不放电，模型把它当放电神经元）。页面上的「记忆 → 转向的桥」是手写的、默认关 |
